...
| C | DOT traffic counts integrated honestly? | **YES** | Step 5 (Street level) shows the 23 sampled matched locations (filled squares) and states that the strict matched-month analysis found no eligible pairs, so there is no citywide street estimate. The card lists the five same-month 2024→2025 pairs; Details covers the older / different-month tiers and the post-toll-only Deegan / Bruckner snapshots. Every square's inspector names its tier and the one-week-sample limitation. "All DOT sites" reveals the 291 single-count spots in Explore. No DOT figure is used to claim rerouting. |
