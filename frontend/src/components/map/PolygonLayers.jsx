...
function PolygonLayers() {
  ...
  const { layerVisibility, dacMode, overlayOpacity = 1 } = useAppState();
  ...
  return (
    <>
      <Source id="dac" type="geojson" data={geo.dac}>
        <Layer id="dac-fill" type="fill" beforeId="anchor-lines" paint={{ 'fill-color': dacColor, 'fill-opacity': dacOn ? overlayOpacity : 0, 'fill-opacity-transition': { duration: 600 }, 'fill-antialias': false }} />
      </Source>
      <Source id="uhf" type="geojson" data={geo.uhf42}>
        <Layer id="uhf-fill" type="fill" beforeId="anchor-lines" paint={{ 'fill-color': violetExpression('asthma_ed_children', lo, hi), 'fill-opacity': uhfOn ? overlayOpacity : 0, 'fill-opacity-transition': { duration: 600 }, 'fill-antialias': false }} />
      </Source>
    </>
  );
}
