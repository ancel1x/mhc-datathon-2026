 ...
 * It also times each step's "before -> after" replay (tour or not): a step with a `from` state opens on the
 * earlier year, holds it, fast-forwards to its own year, then labels the result.
 */
export default function GuidePlayer({ guide }) {
  ...
  useEffect(() => {
    if (!sweepId || intro !== 'done') return undefined;
    ...
    return () => window.clearTimeout(t);
  }, [sweepId, sweepPhase, intro, dispatch, reduced]);
