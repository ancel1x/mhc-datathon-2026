...
export const CHAPTERS = [
  ...
  },
  {
    ...
    from: { period: 'pre_2024', metric: 'absolute' }, // opens on 2024, then fast-forwards to 2025
    period: 'post_2025',
    metric: 'change',
    featured: () => null,
  },
  ...
  },
  {
    id: 'burden',
    era: 'Chapter 5',
    when: 'CHAPTER 5 · WHO BEARS THE BURDEN?',
    short: '5. WHO BEARS THE BURDEN?',
    title: 'Chapter 5 — Who Bears the Burden?',
    camera: { center: [-73.94, 40.735], zoom: 9.95 },
    layers: layers(['aq_monitor', 'dac']),
    period: 'post_2025',
    metric: 'change',
    featured: ({ summary }) => ({ aq_monitor: new Set((summary?.reconciled?.air ?? []).filter((r) => r.class !== 'no_baseline').map((r) => r.id)) }),
  },
  ...
  },
  {
    id: 'one_year_later',
    era: 'Chapter 7',
    when: 'CHAPTER 7 · ONE YEAR LATER',
    short: '7. ONE YEAR LATER',
    title: 'Chapter 7 — One Year Later',
    camera: { center: [-73.91, 40.82], zoom: 10.75 },
    layers: layers(['bt_facility', 'flow', 'aq_monitor']),
    from: { period: 'post_2025', metric: 'change' }, // opens on year one, then fast-forwards to 2026
    period: 'post_2026_ytd',
    metric: 'change',
    featured: () => ({
      bt_facility: new Set(['cross_bay', 'qmt', 'rfk_bronx']),
      aq_monitor: new Set(['aq_36061NY08552', 'aq_36005NY11534', 'aq_36005NY12387']),
    }),
  },
  {
    id: 'nyc_become',
    era: 'Chapter 8',
    when: 'CHAPTER 8 · WHAT COULD NYC BECOME?',
